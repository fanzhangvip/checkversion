...
public class DownloadTask extends Handler {
    ...
    private final int MSG_ERROR = 5;//出错了

    private DownloadListner mListner;//下载回调监听

    DownloadTask(FilePoint point, DownloadListner listner) {
        this.mPoint = point;
    ...
    public synchronized void start() {
        try {
            ...
            isDownloading = true;
            mHttpUtil.getContentLength(mPoint.getUrl(), new com.squareup.okhttp.Callback() {
                @Override
                public void onResponse(Response response) throws IOException {
                    if (response.code() != 200) {
                        close(response.body());
                        resetStutus();
                        sendMessage(MSG_ERROR, "网络不支持断点续传", DownloadListner.ERROR_NOT_206);
                        return;
                    }
                    // 获取资源大小
                    mFileLength = response.body().contentLength();
                    VLogOutput.d(TAG, "mFileLength: " + mFileLength);
                    close(response.body());
                ...
                public void onFailure(Request request, IOException e) {
                    VLogOutput.d(TAG, e.getLocalizedMessage());
                    resetStutus();
                    sendMessage(MSG_ERROR, "网络出错了", DownloadListner.ERROR_RESPONSE_FAIL);
                }
            });
        } catch (IOException e) {
            VLogOutput.d(TAG, e.getLocalizedMessage());
    ...
    private void setDownloadError(int errorCode, final String error) {
        if (mListner != null) {
            mListner.onDownloadError(errorCode, error);
        }
    }
    ...
    public void download(final long startIndex, final long endIndex, final int threadId) throws IOException {
        ...
        mHttpUtil.downloadFileByRange(mPoint.getUrl(), finalStartIndex, endIndex, new com.squareup.okhttp.Callback() {
            @Override
            public void onResponse(Response response) throws IOException {
                ...
                FileChannel channelOut = tmpAccessFile.getChannel();
                MappedByteBuffer mappedBuffer = channelOut.map(FileChannel.MapMode.READ_WRITE, finalStartIndex, response.body().contentLength());
                  /*  将网络流中的文件写入本地*/
                ...
                //关闭资源
                close(is, channelOut, cacheAccessFile, response.body());
                // 删除临时文件
                cleanFile(cacheFile);
                //发送完成消息
            ...
            @Override
            public void onFailure(Request request, IOException e) {
                VLogOutput.d(TAG, "download: " + e.getLocalizedMessage());
                isDownloading = false;
                sendMessage(MSG_ERROR, "网络出错了", DownloadListner.ERROR_RESPONSE_FAIL);
            }
        });
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_PROGRESS://进度
                ...
                mListner.onDownloadProgress(progress * 1.0f / mFileLength);
                break;
            case MSG_PAUSE://暂停
                childPauseCount++;
                if (childPauseCount % THREAD_COUNT != 0) return;//等待所有的线程完成暂停，真正意义的暂停，以下同理
                resetStutus();
                mListner.onDownloadPause();
                break;
            case MSG_FINISH://完成
                childFinishCount++;
                if (childFinishCount % THREAD_COUNT != 0) return;
                File apk = new File(mPoint.getFilePath(), mPoint.getFileName());
                mTmpFile.renameTo(apk);//下载完毕后，重命名目标文件名
                resetStutus();
                mListner.onDownloadFinished(apk.getAbsolutePath());
                break;
            case MSG_CANCEL://取消
                childCanleCount++;
                ...
                mListner.onDownloadCancel();
                break;
            case MSG_ERROR:
                final String message = (String) msg.obj;
                int errorCode = msg.arg1;
                setDownloadError(errorCode, message);
                break;
        }
    }
    ...
    private void sendMessage(int what, String msg, int errorCode) {
        //发送暂停消息
        Message message = new Message();
        message.what = what;
